...
type
  TformNivel2 = class(TForm)
    ...
    btnReiniciar: TPanel;
    btnRegresar: TPanel;
    GanadorPartida: TMediaPlayer;
    TuPerdiste: TMediaPlayer;
    TimerTablero: TTimer;
    TimerEdit: TTimer;
    ...
    procedure btnRegresarMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure TimerLosingTimer(Sender: TObject);
  private
...
var
  formNivel2: TformNivel2;
        ruta:string;
        i:Integer;

implementation

uses question1, configuration, Level1;

{$R *.dfm}

procedure TformNivel2.btnGenerarClick(Sender: TObject);
begin
  Randomize;
  ruta:=ExtractFilePath(Application.ExeName);

  for i:=0 to 4 do
    ListCartas.Items.Add(IntToStr( Random(99) + 1 ));
...
procedure TformNivel2.btnConfigClick(Sender: TObject);
begin
    formConfig.Show;
end;

procedure TformNivel2.TimerTableroTimer(Sender: TObject);
begin
    P_Principal.Visible:=False;
    L_TextoInfo.Visible:=True;
end;

procedure TformNivel2.btnAgregarClick(Sender: TObject);
begin
...
procedure TformNivel2.btnComprobarClick(Sender: TObject);
begin
  memoComprobar.Clear;
  for i:=0 to ListCartas.Count-1 do
    ...
    if  (ListComprobar.Items.Text) = (ListCartas.Items.Text) then begin
        ...
        R4.Picture.LoadFromFile(ruta+'\fotos\'+memoCartas.Lines[3]+'.png');
        R5.Picture.LoadFromFile(ruta+'\fotos\'+memoCartas.Lines[4]+'.png');
        TimerLosing.Enabled:=True;
        TuPerdiste.Open;
        TuPerdiste.Play;
        //formNiveles.Nivel1:=0;
...
procedure TformNivel2.TimerEditTimer(Sender: TObject);
begin
    E1.Visible:=True;
    E2.Visible:=True;
    E3.Visible:=True;
...
procedure TformNivel2.TimerCerrarTimer(Sender: TObject);
begin
...
procedure TformNivel2.AbrirPreguntaTimer(Sender: TObject);
begin
    formPregunta1.Show;
end;

procedure TformNivel2.btnReiniciarClick(Sender: TObject);
begin
    ...
    btnComprobar.Visible:=False;
        ...
        TuPerdiste.Enabled:=False;
        TuPerdiste.Close;
end;

procedure TformNivel2.btnReiniciarMouseMove(Sender: TObject;
...
procedure TformNivel2.btnRegresarClick(Sender: TObject);
begin
    btnReiniciar.Enabled:=False;
    formNivel2.Close;
end;

procedure TformNivel2.btnRegresarMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TformNivel2.TimerLosingTimer(Sender: TObject);
begin
    TuPerdiste.Enabled:=True;
end;
